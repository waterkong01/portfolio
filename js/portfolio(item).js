$(document).ready(function() {
    ...
    const selectedItem = NEW_ITEM_LIST.find(item => item.item_no === parseInt(itemId));
    ...
    // 맨 처음 8개의 li를 만드는 코드
    let count = 0;
    function init(qty) {  // 매개변수 인자 qty 받아서 몇개씩 찍어줄지 결정
        let el_ul = $('.item_list_box'); //요소 잡기
        for (let i = 0; i < qty; i++) {
            // 각 아이템에 고유한 ID를 추가
            let itemId = imagePaths[i].item_no;
            
            // 각 아이템을 클릭하면 해당 아이템의 세부 정보를 보여주는 페이지로 이동하는 링크를 생성
            let item = `<a href="bath_collection_detail.html?item_id=${itemId}" class="item_box_a">
                            <li class="item_box">
                                <div class="item_img">
                                    <img src="img/${imagePaths[i].src}" class="item_img${i}" alt="상품 이미지">
                                </div>
                                <div class="item_title">${imagePaths[i].name}</div>
                                <div class="s_price">${imagePaths[i].size_s}</div>
                                <div class="r_price">${imagePaths[i].size_r}</div>
                                <div class="l_price">${imagePaths[i].size_l}</div>
                                <div class="s_price">${imagePaths[i].size_s}</div>
                                <div class="m_price">${imagePaths[i].size_m}</div>
                                <div class="o_price">${imagePaths[i].size_o}</div>
                            </li>
                        </a>`;
            $(item).appendTo(el_ul);
        }
