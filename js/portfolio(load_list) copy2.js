...
function getMenuInfo(item_no) {
    ...
    return imagePaths.find(item => item.item_no === item_no);
}
...
function handleImageClick(item_no) {
    const selectedItem = imagePaths.find(item => item.item_no === item_no);
    if (selectedItem) {
        // 페이지 이동
        window.location.href = `portfolio(item) copy.html?item_no=${selectedItem.item_no}`;
    }
}
...
function handleImageClick(item_no) {
    // 여기에서 item_no를 이용하여 선택한 상품의 메뉴 정보를 가져옵니다.
    const selectedItem = getMenuInfo(item_no);

    if (selectedItem) {
        // 선택한 메뉴 정보를 사용하여 화면에 표시
        displayMenuInfo(selectedItem);
    } else {
...
// 메뉴 정보를 화면에 표시하는 함수
function displayMenuInfo(menuItem) {
    // 메뉴 이미지 표시
    const menuImage = document.getElementById('menuImage');
    menuImage.src = menuItem.imageURL;
    ...
    menuPrice.textContent = `가격: ${menuItem.price}원`;
}
...
imagePaths.forEach((imageInfo, index) => {
    const menuItem = document.createElement('div');
    menuItem.classList.add('menu-item');
    menuItem.dataset.category = imageInfo.category; // data-category 속성 설정
    const img = document.createElement('img');
    img.src = imageInfo.path;
    img.alt = `Menu ${index + 1}`;
    menuItem.appendChild(img);
    menuContainer.appendChild(menuItem);
});

// 네비게이션 바 항목을 클릭했을 때의 동작을 정의
document.querySelectorAll('.navigation a').forEach(function(navItem) {
    navItem.addEventListener('click', function(event) {
        ...
        // 모든 네비게이션 항목의 스타일 초기화
        document.querySelectorAll('.navigation a').forEach(function(item) {
            item.classList.remove('active');
        });
        
        // 선택한 네비게이션 항목에 'active' 클래스 추가하여 색상 변경
        this.classList.add('active');
    });
});


// 네비게이션 바의 카테고리 버튼을 클릭할 때 호출되는 함수
function filterImagesByCategory(category) {
    // 모든 이미지 요소를 가져옵니다.
    const menuItems = document.querySelectorAll('.menu-item');

    // 각 이미지를 순회하며 선택한 카테고리와 일치하는지 확인합니다.
    menuItems.forEach((menuItem) => {
        const imageCategory = menuItem.dataset.category;

        if (category === 'all' || category === imageCategory) {
            menuItem.style.display = 'block'; // 선택한 카테고리와 일치하면 표시
        } else {
            menuItem.style.display = 'none'; // 선택한 카테고리와 다르면 숨김
        }
    });
}

// 네비게이션 바의 카테고리 버튼을 클릭할 때 필터링 함수를 호출하도록 이벤트 리스너를 추가합니다.
document.querySelectorAll('.navigation a').forEach((navItem) => {
    navItem.addEventListener('click', function (event) {
        event.preventDefault(); // 기본 동작 방지
        const category = this.getAttribute('data-category'); // 클릭한 항목의 카테고리를 가져옴
        filterImagesByCategory(category); // 해당 카테고리의 이미지만 표시하는 함수 호출
    });
});
